...
const PLANNER_RESPONSE_EXAMPLE = {
  ...
  playbook: {
    ...
    // NOTE: playbook.ctas is injected server-side after Gemini responds (not AI-generated).
    // Schema: [{ type: string, priority: number, meta?: object }]
  }
};
...
// Generates deterministic CTAs from active modules + RSVP signals — NOT by Gemini.
// priority: lower = shown first in the Overview.
function buildPlanCtas(activeModules, guestRsvpSignals) {
  const mods = ensureObject(activeModules);
  const signals = normalizeArray(guestRsvpSignals);
  const ctas = [];

  if (mods.megaphone) {
    ctas.push({ type: "megaphone", priority: 1 });
  }
  if (mods.accommodation) {
    const needsCount = signals.filter((s) => ensureObject(s).needsAccommodation === true).length;
    ctas.push({ type: "accommodation", priority: 2, meta: { needsCount } });
  }
  if (mods.transport) {
    const modes = {};
    signals.forEach((s) => {
      const mode = normalizeText(ensureObject(s).transportMode);
      if (mode) modes[mode] = (modes[mode] || 0) + 1;
    });
    ctas.push({ type: "transport", priority: 3, meta: { modes } });
  }
  if (mods.finance) {
    ctas.push({ type: "finance", priority: 4 });
  }
  if (mods.spotify) {
    ctas.push({ type: "spotify", priority: 5 });
  }
...
router.post("/planner", async (req, res) => {
  ...
  try {
    ...
    // Inject deterministic CTAs — not generated by Gemini, always current.
    const guestRsvpSignals = normalizeArray(eventContext.guestRsvpSignals);
    const ctas = buildPlanCtas(activeModules, guestRsvpSignals);
    if (data && data.playbook) {
      data.playbook.ctas = ctas;
    }
